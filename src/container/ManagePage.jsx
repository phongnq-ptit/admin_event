...
import { setSaleOff, setUser } from '../store/Module.action';
...
function ManagePage() {
    ...
    const handleSearch = () => {
        const newData = tmp.current.filter(
            item => (item.name + '').toLocaleLowerCase().includes(dataSearch.name.toLocaleLowerCase()) &&
